...
from model.symple_mf import MF
from model.NCF import NCF
from model.symple_lr import lr
...
class Model():
    ...
    def load_model(self, checkpoint_name=None):
        ...
        if checkpoint_name != "q":
            checkpoint = torch.load(os.path.join(self.result_dir, checkpoint_name))
    ...
    def init_model(self, load_checkpoint=False, checkpoint_name=None):
        ...
        self.model.reset_parameters()
        num_epoch = 0
        if load_checkpoint and checkpoint_name in os.listdir(os.path.join(self.result_dir)):
            checkpoint = torch.load(os.path.join(self.result_dir, checkpoint_name))
            num_epoch = checkpoint["epoch"]
            self.reset_train_dataset(checkpoint['remain_ids'])
            self.performance = checkpoint["performance"]
    ...
    def train(self, num_epoch=180000, load_checkpoint=True, save_checkpoints=True, verbose=False,
        ...
        if save_checkpoints and checkpoint_name not in os.listdir(os.path.join(self.result_dir)):
            torch.save({
                'epoch': num_epoch,
                'remain_ids': self.remain_ids,
                'performance': self.performance,
                'model_state_dict': self.model.state_dict()
                }, os.path.join(self.result_dir, checkpoint_name))
    ...
    def plot_loss_of_train_process(self):
        ...
        """
        plt.plot(self.performance["train_loss"], label="train loss")
        plt.plot(self.performance["test_loss"], label="test loss")
        plt.legend()
        plt.show()
